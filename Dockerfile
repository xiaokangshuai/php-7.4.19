...
RUN set -eux; \
        addgroup -g 82 -S fbzs; \
        adduser -u 82 -D -S -G fbzs fbzs
...
RUN set -eux; \
        ...
        if [ -d php-fpm.d ]; then \
                ...
                sed -i  's/user = www-data/user = fbzs/g' php-fpm.d/www.conf ; \
                sed -i  's/group = www-data/group = fbzs/g' php-fpm.d/www.conf ; \
        else \
...
COPY phpredis-4.0.0 /usr/src/php/ext/redis  
RUN docker-php-ext-install redis 
#安装xlswriter扩展
RUN mkdir -p /usr/src/php/ext/xlswriter 
COPY xlswriter-1.3.6 /usr/src/php/ext/xlswriter
RUN docker-php-ext-install xlswriter
EXPOSE 9000
CMD ["php-fpm"]
